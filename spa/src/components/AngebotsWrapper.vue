             ...
             <v-row  class="PostSelectWrapper" row wrap>
               ...
               <v-col md="3"> 
                   <Select 
                        ...
                        <v-col class="SearchButtonWrapper" md="2">  <v-btn class="SearchButton" @click="fetchCategory()" dark> SEARCH </v-btn></v-col>
        ...
        async fetchCategory(){
            ...
            try{
                const {data} = await axios.get(
                    'https://karlsruher-pass.de/wp-json/wp/v2/categories?search='+ this.selectCity
                )
            ...
            try{
                const {data} = await axios.get(
                    'https://karlsruher-pass.de/wp-json/wp/v2/categories?search='+ this.selectCategory
                )
                data.forEach(element => {
            ...
            try{
                const {data} = await axios.get(
                    'https://karlsruher-pass.de/wp-json/wp/v2/categories?search='+ this.selectPass
                )
                data.forEach(element => {
